...
static void gatts_connect_cb(const vm_bt_gatt_connection_t *conn, VMBOOL connected, const vm_bt_gatt_address_t *bd_addr);
...
void gatt_start(void)
{
    ...
    if (g_gatt_cntx.gatt_status == GATT_STATUS_DISABLED)
	{
	    ...
	    vm_bt_gatt_server_register((const VMUINT8 *)&g_gatt_cntx.uuid, &g_gatts_cb);
    }
...
void btcm_cb(VM_BT_CM_EVENT evt, void* param, void* user_data)
{
                          ...
                          VM_BT_GATT_ATTRIBUTE_HANDLE srvc_handle)
{
    ...
    if ((g_gatt_cntx.context_handle == context_handle) && (memcmp(srvc_id, g_battery_srv_uuid_ptr, sizeof(vm_bt_gatt_service_info_t)) == 0))
	{
		...
		if (g_gatt_cntx.gatt_status == GATT_STATUS_ENABLING)
		{
			if (status == 0)
			{
                vm_log_info("rhb [Battery GATT] gatt_op_flag:%d", g_gatt_cntx.gatt_op_flag);
				if(g_gatt_cntx.gatt_op_flag == GATT_OP_ADD_SERVICE)
				{
					...
					vm_bt_gatt_server_add_characteristic(context_handle, srvc_handle, &(g_battery_char_uuid_ptr->uuid),
                        VM_BT_GATT_CHAR_PROPERTY_READ | VM_BT_GATT_CHAR_PROPERTY_WRITE, VM_BT_GATT_PERMISSION_WRITE | VM_BT_GATT_PERMISSION_READ /*| VM_BT_GATT_PERMISSION_READ_ENCRYPTED | VM_BT_GATT_PERMISSION_READ_ENC_MITM*/ );
				}
			}
			else
			{
    ...
    else
    {
		...
		if (status == 0)
		{
			...
			static VMUINT8 myUUID[] = {0xFB, 0x34, 0x9B, 0x5F, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0x19, 0x2A, 0x00, 0xFF};
			...
			std::function<const int()> myhook = [&] () { return myReadHook(); } ;
			myChar->setReadHook(myhook);

			//myChar->registerMe(context_handle, srvc_handle);
			myService.addCharacteristic(myChar);

			static VMUINT8 myc2UUID[] = {0xFC, 0x35, 0x9B, 0x5F, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0x19, 0x2A, 0x02, 0xFD};
			c2 = new GATTCharacteristic(myc2UUID,
					VM_BT_GATT_CHAR_PROPERTY_READ | VM_BT_GATT_CHAR_PROPERTY_WRITE,
					VM_BT_GATT_PERMISSION_WRITE | VM_BT_GATT_PERMISSION_READ);

			// c2->registerMe(context_handle, srvc_handle);
                                 ...
                                 VM_BT_GATT_ATTRIBUTE_HANDLE srvc_handle, VM_BT_GATT_ATTRIBUTE_HANDLE char_handle)
{
    ...
    // rhb: lookup char, initialize the char_handle within
...
void gatts_connect_cb(const vm_bt_gatt_connection_t *conn, VMBOOL connected, const vm_bt_gatt_address_t *bd_addr)
{
...
#ifdef INCLASS
// presumably could just be GATTCharacteristic->readMe();
static void gatts_read_rhb_response(vm_bt_gatt_connection_t *conn, VMUINT16 trans_id, VM_BT_GATT_ATTRIBUTE_HANDLE attr_handle, VMUINT16 offset)
{
    vm_bt_gatt_attribute_value_t att_value;

    vm_log_info("rhb response()");
